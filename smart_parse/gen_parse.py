...
from utils import aget_response, get_response
import asyncio
...
def clean_to_json(response: str) -> dict:
    ...
    for match in matches:
        try:
            ...
            continue
    raise RuntimeError(
        f"API's result does not contain a valid json object, returned: {response}"
    )


def get_parse_results(email: dict, max_patience: int = 5, **kwargs) -> dict:
    prompt = get_prompt(email, **kwargs)
    raw_result = get_response(prompt, max_patience)
    result = clean_to_json(raw_result)
    return result


async def aget_parse_results(
    email: dict,
    max_patience: int = 5,
    ...
    **kwargs,
    ...
    prompt = get_prompt(email, **kwargs)
    raw_result = await aget_response(prompt, max_patience, semaphore=semaphore)
    result = clean_to_json(raw_result)
    return result
...
def gen_predictions(src_path, tgt_path, n_parallel: int = 0, **kwargs):
    ...
    if n_parallel == 0:
        ...
        async def async_get_results(i, email):
            async with sem:
                print("working on email", i + 1, "total", len(data))
                result = await aget_parse_results(email, semaphore=sem, **kwargs)
...
if __name__ == "__main__":
    ...
    gen_predictions(
        r"smart_parse\data\origin-email.json",
        r"smart_parse\predictions\time_and_tz-0614-openai-update.json",
        n_parallel=20,
